...
def compute_loss(model, posenet, segnet, comp_a, comp_b, gt_a, gt_b, w_seg, w_temp):
    ...
    pair_inf = torch.stack([inf_a.permute(0, 2, 3, 1).contiguous(),
                            inf_b.permute(0, 2, 3, 1).contiguous()], dim=1)
    pair_gt = torch.stack([gt_a.permute(0, 2, 3, 1).contiguous(),
                           gt_b.permute(0, 2, 3, 1).contiguous()], dim=1)

    posenet_in_inf = posenet.preprocess_input(pair_inf.permute(0, 1, 4, 2, 3).contiguous())
    with torch.no_grad():
        posenet_in_gt = posenet.preprocess_input(pair_gt.permute(0, 1, 4, 2, 3).contiguous())
    ...
    segnet_in_inf = segnet.preprocess_input(pair_inf.permute(0, 1, 4, 2, 3).contiguous())
    with torch.no_grad():
        segnet_in_gt = segnet.preprocess_input(pair_gt.permute(0, 1, 4, 2, 3).contiguous())
...
def train(args):
    ...
    archive_candidates = [
        os.path.join(HERE, 'archive/0.mkv'),
        os.path.join(PD, 'submissions/autoresearch/archive/0.mkv'),
    ]
    archive_path = None
    ...
    # Subsample frames to reduce memory (every 6th frame for low-RAM environments)
    SUBSAMPLE = 6
